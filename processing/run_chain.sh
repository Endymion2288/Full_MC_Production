...
CMSSW_12_BASE="${CMSSW_12_BASE:-/cvmfs/cms.cern.ch/el8_amd64_gcc10/cms/cmssw/CMSSW_12_4_14_patch3}"
...
make_remote_dir() {
    ...
    run_xrdfs "${EOS_HOST}" mkdir -p "${EOS_PATH_BASE}/${remote_subpath}" || {
...
get_lhe_file() {
    ...
    # List files via xrdfs (use clean environment to avoid CMSSW conflicts)
    local file_list
    file_list=$(run_xrdfs "${EOS_HOST}" ls "${EOS_PATH_BASE}/${pool_subpath}" 2>/dev/null | grep '\.lhe$' | sort)
...
ensure_cmssw12_project() {
    local project_dir="${WORKDIR}/CMSSW_12_4_14_patch3"
    ...
    msg_info "Creating CMSSW_12_4_14_patch3 project from CVMFS..." >&2
    source /cvmfs/cms.cern.ch/cmsset_default.sh
    export SCRAM_ARCH=el8_amd64_gcc10
    
    cd "${WORKDIR}"
    scramv1 project CMSSW CMSSW_12_4_14_patch3 >&2 || {
        msg_error "Failed to create CMSSW_12_4_14_patch3 project" >&2
        return 1
    }
    cd - > /dev/null
...
run_shower() {
    ...
    for ((i=0; i<n_files; i++)); do
        ...
        if [[ "$mode" == "phi" ]]; then
            ...
            ./shower_phi "${lhe_file}" "${hepmc_output}" "${shower_events}" 3.0 0 0 5000
        else
            # Standard shower: muon pT > 2.5 GeV, |eta| < 2.4
            ./shower_normal "${lhe_file}" "${hepmc_output}" "${shower_events}" 0 0 5000
        fi
...
Input format examples:
  ...
  GEN:pool_jpsi_CSCO_g:0      LHE generated by DAG

Examples:
  # JJP DPS: Two J/psi sources mixed
  $0 --inputs pool_jpsi_CSCO_g:0,pool_jpsi_CSCO_g:1 --modes normal,phi \\
     --analysis JJP --campaign JJP_DPS1 --job-id 0
EOF
    exit 1
}
