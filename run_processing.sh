...
# Ensure libssl.so.1.1 is available for CMSSW_12_4_14
export LD_LIBRARY_PATH="/usr/lib64:${LD_LIBRARY_PATH:-}"

echo "Running processing chain..."
cd runtime/processing
if ! bash run_chain.sh --inputs "${INPUTS}" --modes "${MODES}" --analysis "${ANALYSIS}" --campaign "${CAMPAIGN}" --job-id "${JOB_ID}" --max-events "${MAX_EVENTS}" --enable-ntuple "${ENABLE_NTUPLE}" --cleanup "${CLEANUP}"; then
